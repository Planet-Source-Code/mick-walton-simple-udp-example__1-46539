...
' UDP has no active connections, so a connection is never made between two hosts.
' Instead you set the destination and then send the packets.


'Set the destination host and port for this connection
...
Else
Addtolog "You didnt set remote host/port"
End If
End Sub
